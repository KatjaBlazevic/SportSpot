...
const SVE_KVARTOVI = ["Svi kvartovi", "Rujevica", "Pećine", "Mlaka", "Zamet", "Kantrida", "Trsat", "Centar", "Kozala"];
const PERIODI = [
  ...
  return (
    ...
    >
      <div style={{ display: "flex", gap: 16, padding: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          <h3 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: "#1D4ED8", fontFamily: "'DM Sans', sans-serif", cursor: "pointer" }}>{objekt.naziv}</h3>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4, color: "#6B7280", fontSize: 13 }}>
            ...
            {objekt.kvart}, Rijeka
            {objekt.ocjena && (<><span style={{ marginLeft: 6 }}>⭐ {objekt.ocjena}</span><span style={{ color: "#9CA3AF" }}>({objekt.brojRecenzija} recenzija)</span></>)}
          </div>
        ...
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          ...
          <div style={{ fontSize: 24, fontWeight: 800, color: "#111827", fontFamily: "'DM Sans', sans-serif" }}>
            {objekt.cijenaOd === 0 ? <span style={{ color: "#16A34A", fontSize: 18 }}>Besplatno</span> : <>{objekt.cijenaOd}€<span style={{ fontSize: 13, fontWeight: 500, color: "#6B7280" }}>/h</span></>}
          </div>
        </div>
      </div>
      ...
      {slobodniTermini.length > 0 && (
        <div style={{ borderTop: "1px solid #F3F4F6" }}>
          <button onClick={() => setProsiren(!prosiren)}
            ...
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ transform: prosiren ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}><path d="m6 9 6 6 6-6" /></svg>
          </button>
          {prosiren && (
              ...
              {slobodniTermini.map((termin) => (
                <div key={termin.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", borderRadius: 10, background: "#F8FAFF", border: "1px solid #E8EEFF", marginTop: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 8, background: "#EFF6FF", display: "flex", alignItems: "center", justifyContent: "center" }}><SportIcon sport={objekt.sportovi[0]} /></div>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 14, color: "#111827" }}>
                        {termin.datum && <span style={{ color: "#6B7280", fontWeight: 400 }}>{new Date(termin.datum).toLocaleDateString("hr-HR")} · </span>}
                        {termin.vrijemePocetka} – {termin.vrijemeKraja}
                      </div>
                      <div style={{ fontSize: 12, color: "#6B7280" }}>
                        {termin.cijena === 0 ? <span style={{ color: "#16A34A", fontWeight: 600 }}>Besplatno</span> : <span style={{ color: "#1D4ED8", fontWeight: 600 }}>{termin.cijena}€</span>}
                      </div>
                    </div>
                  </div>
                  <button
...
function MapaPlaceholder({ objekti }: { objekti: Objekt[] }) {
  const pinPoints = objekti.slice(0, 4).map((obj, i) => ({ x: [52, 68, 44, 35][i] ?? 50, y: [38, 58, 62, 48][i] ?? 50, objekt: obj }));
  return (
    <div style={{ position: "sticky", top: 80, height: "calc(100vh - 96px)", borderRadius: 20, overflow: "hidden", background: "#1E293B", border: "1.5px solid #334155", boxShadow: "0 4px 24px rgba(0,0,0,0.15)" }}>
      <svg width="100%" height="100%" viewBox="0 0 400 600" style={{ position: "absolute", inset: 0 }}>
        ...
        {[80, 140, 200, 260, 320, 380, 440, 490].map((y) => <line key={y} x1="0" y1={y} x2="400" y2={y} stroke="#334155" strokeWidth="1.5" />)}
        {[60, 120, 180, 240, 300, 360].map((x) => <line key={x} x1={x} y1="0" x2={x} y2="600" stroke="#334155" strokeWidth="1.5" />)}
        ...
        <div style={{ position: "absolute", bottom: 16, left: 16, right: 16, background: "#fff", borderRadius: 14, padding: 14, boxShadow: "0 8px 32px rgba(0,0,0,0.25)", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3 }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: "#1D4ED8" }}>{objekti[0].cijenaOd === 0 ? "Besplatno" : `${objekti[0].cijenaOd}€/h`}</span>
              {objekti[0].termini.length > 0 && <span style={{ fontSize: 12, color: "#EA580C", fontWeight: 600 }}>⚡ Brza rezervacija</span>}
            </div>
          </div>
        </div>
      )}
      <div style={{ position: "absolute", top: 12, right: 12, background: "rgba(0,0,0,0.5)", color: "#94A3B8", fontSize: 11, padding: "4px 10px", borderRadius: 999, fontWeight: 500 }}>Google karta (uskoro)</div>
    </div>
  );
}
...
export default function Home() {
  ...
  const dohvatiObjekte = useCallback(async () => {
    ...
    try {
      ...
      if (odabraniSportovi.length === 1) params.set("sport", odabraniSportovi[0]);
  ...
  }, [odabraniSportovi, odabraniKvart, odabraniDatum, odabraniPeriod]);
  ...
  const toggleSport = (sport: string) => setOdabraniSportovi((prev) => prev.includes(sport) ? prev.filter((s) => s !== sport) : [...prev, sport]);
    ...
    <div style={{ maxWidth: 1400, margin: "0 auto", padding: "20px 24px", display: "grid", gridTemplateColumns: "220px 1fr 380px", gap: 20, alignItems: "start" }}>
      ...
      <aside style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #E8EEFF", padding: 20, position: "sticky", top: 80, boxShadow: "0 2px 12px rgba(37,99,235,0.06)" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {SVE_SPORTOVI.map((sport) => {
              ...
              return (
                <button key={sport} onClick={() => toggleSport(sport)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", borderRadius: 8, border: "none", background: aktivan ? "#1D4ED8" : "transparent", color: aktivan ? "#fff" : "#374151", fontWeight: aktivan ? 600 : 400, fontSize: 14, cursor: "pointer", textAlign: "left", transition: "background 0.15s", fontFamily: "inherit" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <select value={odabraniKvart} onChange={(e) => setOdabraniKvart(e.target.value)}
            style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1.5px solid #E5E7EB", fontSize: 13, color: "#374151", fontFamily: "inherit", background: "#fff", outline: "none", cursor: "pointer" }}>
            {SVE_KVARTOVI.map((k) => <option key={k} value={k}>{k === "Svi kvartovi" ? "Rijeka - Svi kvartovi" : k}</option>)}
          </select>
        </div>
        ...
        {(odabraniSportovi.length > 0 || odabraniKvart !== "Svi kvartovi" || odabraniDatum || odabraniPeriod) && (
          <button onClick={() => { setOdabraniSportovi([]); setOdabraniKvart("Svi kvartovi"); setOdabraniDatum(""); setOdabraniPeriod(""); }}
      ...
      <main>
        <div style={{ marginBottom: 16 }}>
          ...
          <p style={{ margin: 0, fontSize: 13, color: "#9CA3AF" }}>Rezultati za: Rijeka, Hrvatska{searchQuery && ` · "${searchQuery}"`}</p>
        </div>

        {greska && (
          <div style={{ background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 12, padding: "14px 16px", marginBottom: 16, color: "#DC2626", fontSize: 14 }}>⚠️ {greska}</div>
        )}
