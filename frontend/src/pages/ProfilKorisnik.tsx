...
useEffect(() => {
    if (aktivnaTab === "rezervacije" && token) {
      ...
      fetch("http://localhost:5000/api/moje-rezervacije", {
  ...
  const otvoriModal = async () => {
    try {
      ...
      setEditData({
        ime: data.ime || korisnik?.ime || "",
        prezime: data.prezime || korisnik?.prezime || "",
        email: data.email || korisnik?.email || "",
  ...
  const spremiPromjene = async () => {
    if (editData.novaLozinka && editData.novaLozinka !== editData.potvrdaLozinke) {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/profile/update", {
        ...
        body: JSON.stringify({
          ...
          brojMobitela: editData.brojMobitela || null,
          novaLozinka: editData.novaLozinka || null,
        }),
      });
      ...
      if (res.ok) {
        osvjezi({ ...korisnik!, ime: editData.ime, prezime: editData.prezime, email: editData.email });
        setShowModal(false);
      } else {
        setPoruka(data.greska || "Greška pri spremanju.");
      }
    } catch {
  ...
  const ukloniOmiljeni = (id: number) => {
    const novi = omiljeni.filter((o) => o.id !== id);
    setOmiljeni(novi);
    localStorage.setItem("sportspot_omiljeni", JSON.stringify(novi));
  };
  ...
  const formatirajDatum = (datum: string) => {
    ...
    return `${dan}. ${mjeseci[d.getMonth()]} ${d.getFullYear()}.`;
  };

  const getStatusBoja = (status: string) => {
    if (status === "Potvrđeno") return { bg: "#D1FAE5", color: "#059669" };
    if (status === "Odbijeno") return { bg: "#FEE2E2", color: "#DC2626" };
    ...
    <div style={{ minHeight: "calc(100vh - 64px)", background: "#F5F7FA", padding: "32px 16px" }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          borderRadius: 24, padding: 32, marginBottom: 24, color: "#fff", position: "relative", overflow: "hidden",
        }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 24, position: "relative", zIndex: 1 }}>
            <div style={{
              width: 100, height: 100, borderRadius: "50%", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: 36, fontWeight: 800, color: "#1D4ED8", flexShrink: 0, boxShadow: "0 4px 20px rgba(0,0,0,0.2)",
            }}>
            ...
            <button onClick={otvoriModal} style={{
              padding: "12px 24px", borderRadius: 12, background: "#fff", color: "#1D4ED8",
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
          <button onClick={() => setAktivnaTab("rezervacije")} style={{
            padding: "14px 24px", borderRadius: 14,
            background: aktivnaTab === "rezervacije" ? "#1D4ED8" : "#fff",
            color: aktivnaTab === "rezervacije" ? "#fff" : "#374151",
            fontWeight: 600, fontSize: 15, border: "none", cursor: "pointer", fontFamily: "inherit",
          }}>
            📅 Moje rezervacije
          </button>
          <button onClick={() => setAktivnaTab("omiljeni")} style={{
            padding: "14px 24px", borderRadius: 14,
            background: aktivnaTab === "omiljeni" ? "#1D4ED8" : "#fff",
            color: aktivnaTab === "omiljeni" ? "#fff" : "#374151",
            fontWeight: 600, fontSize: 15, border: "none", cursor: "pointer", fontFamily: "inherit",
          }}>
            ❤️ Omiljeni {omiljeni.length > 0 && (
              <span style={{ marginLeft: 4, padding: "2px 8px", borderRadius: 10, background: aktivnaTab === "omiljeni" ? "rgba(255,255,255,0.2)" : "#FEE2E2", fontSize: 12, color: aktivnaTab === "omiljeni" ? "#fff" : "#EF4444" }}>
                {omiljeni.length}
              </span>
            )}
          </button>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 14 }}>
                {omiljeni.map((obj) => (
                  <div key={obj.id} style={{ padding: 20, borderRadius: 16, border: "1px solid #E5E7EB", position: "relative" }}>
                    ...
                    <button onClick={() => ukloniOmiljeni(obj.id)} style={{
                      position: "absolute", top: 12, right: 12, width: 28, height: 28, borderRadius: "50%",
                      border: "1px solid #FEE2E2", background: "#FEF2F2", color: "#EF4444", fontSize: 14, cursor: "pointer",
          ...
          <div style={{
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: 32, width: "90%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                ...
                <button onClick={zatvoriModal} style={{
                  width: 36, height: 36, borderRadius: "50%", border: "1px solid #E5E7EB", background: "#fff",
                  color: "#6B7280", fontSize: 18, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
                }}>✕</button>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div>
                  ...
                  <input type="tel" value={editData.brojMobitela} onChange={(e) => setEditData({ ...editData, brojMobitela: e.target.value })}
                    placeholder="+385 91 234 5678" style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: "1.5px solid #E5E7EB", fontSize: 15, fontFamily: "inherit", boxSizing: "border-box" }} />
                </div>
                <div style={{ borderTop: "1px solid #E5E7EB", paddingTop: 16, marginTop: 8 }}>
                  ...
                  <input type="password" value={editData.novaLozinka} onChange={(e) => setEditData({ ...editData, novaLozinka: e.target.value })}
                    style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: "1.5px solid #E5E7EB", fontSize: 15, fontFamily: "inherit", boxSizing: "border-box", marginBottom: 12 }} />
                  <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 6 }}>Potvrdi lozinku</label>
                  <input type="password" value={editData.potvrdaLozinke} onChange={(e) => setEditData({ ...editData, potvrdaLozinke: e.target.value })}
                    style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: "1.5px solid #E5E7EB", fontSize: 15, fontFamily: "inherit", boxSizing: "border-box" }} />
                </div>
