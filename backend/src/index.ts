...
app.post("/api/objekti", async (req: Request, res: Response) => {
  ...
  if (!authHeader?.startsWith("Bearer ")) { res.status(401).json({ greska: "Niste prijavljeni." }); return; }
  const token = authHeader.split(" ")[1];
  if (!token) { res.status(401).json({ greska: "Neispravan token." }); return; }
  ...
  try {
    ...
    const [result] = await pool.query(
      ...
      [decoded.id, naziv, adresa, kvart, kapacitet || null, opis || null, slikaUrl || null, lat ?? null, lng ?? null],
    );
...
app.put("/api/objekti/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (rows.length === 0) { res.status(404).json({ greska: "Objekt ne postoji ili nemate pravo uređivati." }); return; }
...
app.get("/api/profile", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (korisnici.length === 0) { res.status(404).json({ greska: "Korisnik ne postoji." }); return; }
    const k = korisnici[0];
    if (!k) { res.status(404).json({ greska: "Korisnik ne postoji." }); return; }
...
app.post("/api/profile/update", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (email && email !== current.Email) {
      ...
      if (rows.length > 0) { res.status(409).json({ greska: "Email je već u uporabi." }); return; }
    }
